...
const Signup = withSession(
  wrapper(async (req, res) => {
    ...
    const verify = await axios({
      ...
      data: JSON.stringify({ email: user.email, address: {name: `${req.body.username}_Magic_${user.publicAddress.substring(0,5)}...${user.publicAddress.substring(18)}`, content:user.publicAddress, chain: 'Ethereum'}, ...req.body }),
      headers: {
